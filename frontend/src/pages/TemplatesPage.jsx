  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {showForm && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                placeholder="E.g. Weekly Product Highlight"
                value={form.name}
          ...
          <div className="mb-4">
            ...
            <textarea
              className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm resize-none h-32 focus:outline-none focus:border-purple-500"
      ...
      {!isLoading && templates.length === 0 && (
        <p className="text-gray-500 text-center py-16">No templates yet. Create one to reuse captions quickly.</p>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {templates.map(tpl => (
          <div key={tpl.id} className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col gap-3">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                <FileText size={16} className="text-purple-400"/>
                <span className="font-medium text-sm">{tpl.name}</span>
              </div>
              <span className="text-xs bg-white/10 text-gray-300 px-2 py-0.5 rounded-full capitalize">{tpl.channel}</span>
            </div>
            ...
            <div className="flex gap-2 mt-auto">
              ...
              ><ArrowRight size={12}/> Use Template</button>
              <button
